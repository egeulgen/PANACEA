...
#' res <- score_drugs_RWR_based(driveR_res = toy_data,
...
score_drugs_RWR_based <- function(driveR_res, drug_interactions_df, W_mat,
    ...
    processed_interactions_df <- process_drug_target_interactions(drug_target_interactions = drug_interactions_df,
    ...
    all_drugs <- unique(processed_interactions_df$drug_name)
     ...
     ### process adj. mat.
    if (verbose)
        cat("Processing adjacency matrixs\n")
